...
/**
 * Provides a parsing for each ADC command received from client, and makes the
 * states transitions Updates all information and ensures stability.
 * 
 * 
 * @author Pietricica
 */
...
public class Command
{
	...
	private void sendUsersInfs()
	{
		...
		{
			if (iterator.cur_client.userok == 1
					&& iterator.cur_client != cur_client)
	...
	void completeLogIn() throws STAException
	{
		...
		if (!cur_client.reg.key) // DO NOT increase HR count and put RG field
		// to 1
		{
			// cur_client.HR=String.valueOf(Integer.parseInt(cur_client.HR)+1);
			cur_client.CT = "2";
		} else
		// DO NOT increase HO count and put OP field to 1
		{
		...
		/*
		 * IoSession [] x= Main.Server.SM.getSessions().toArray(new
		 * IoSession[0]); String inf="\n"; for(int i=0;i<x.length;i++) {
		 * ClientNod tempy=((ClientHandler)(x[i].getAttachment())).myNod;
		 * if(tempy.cur_client.userok==1 && !tempy.cur_client.equals
		 * (cur_client)) //if the user has some inf ... [ meaning he is ok]
		 * inf=inf.substring(0,inf.length()-1)+tempy.cur_client.getINF
		 * ()+"\n\n"; }
		 */
		sendUsersInfs();

		cur_client.sendToClient("BINF DCBA ID" + Vars.SecurityCid + " NI"
				+ ADC.retADCStr(Vars.bot_name) + " CT5 DE"
				+ ADC.retADCStr(Vars.bot_desc));
		cur_client.putOpchat(true);
		cur_client.sendToClient(cur_client.getINF()); // sending inf about
		// itself too
		// cur_client.sendToClient(inf);

		// ok now must send INF to all clients
		Broadcast.getInstance()
				.broadcast(cur_client.getINF(), cur_client.myNod);
		...
		cur_client
				.sendFromBot(cur_client.reg.HideMe ? "You are currently hidden."
						: "");
	...
	void handleINF() throws CommandException, STAException
	{
		...
		// {
		// StringTokenizer inftok=new
		// StringTokenizer(cur_client.cur_inf.substring(9));
		...
		while (tok.hasMoreElements())
		{
			...
			} else if (aux.startsWith("NI"))
			{
				...
				{
					new STAError(cur_client, 200 + Constants.STA_NICK_INVALID,
			...
			} else if (aux.startsWith("I4"))
			{
				...
				if (aux.substring(2).equals("0.0.0.0")
						|| aux.substring(2).equals("localhost"))// only if
				// active client
				{
					cur_client.I4 = cur_client.RealIP;
				}

				else if (!aux.substring(2).equals(cur_client.RealIP)
						&& !aux.substring(2).equals("")
						&& !cur_client.RealIP.equals("127.0.0.1"))
				{
					new STAError(cur_client, 200 + Constants.STA_INVALID_IP,
							"Wrong IP address supplied.", "I4",
							cur_client.RealIP);
					return;
				}
			...
			{
				// new
				// STAError(cur_client,200+Constants.STA_GENERIC_PROTOCOL_ERROR,"Protocol
				// Error.");
				// return ;
				cur_inf = cur_inf + " " + aux;
			}
		...
		if (cur_client.myban != null) // banned
		{
			if (cur_client.myban.time == -1)
			{
				...
				new STAError(cur_client,
						200 + Constants.STA_PERMANENTLY_BANNED, msg);

				return;
			}
			long TL = System.currentTimeMillis() - cur_client.myban.timeofban
					- cur_client.myban.time;
			TL = -TL;
			if (TL > 0)
			{
			...
			if (!temp.cur_client.equals(cur_client))
			{
				if (temp.cur_client.userok == 1)
					if (temp.cur_client.NI.toLowerCase().equals(
							cur_client.NI.toLowerCase())
							&& !SimpleHandler.Users.containsKey(cur_client.ID))
					{
						new STAError(cur_client,
				 ...
				 * temp.cur_client.CIDsecure) { new
				 * STAError(cur_client,200+Constants.STA_CID_TAKEN,"CID taken.
				 * Please go to Settings and pick new PID."); return; }
				 */
		...
		if (AccountsConfig.nickReserved(cur_client.NI, cur_client.ID))
		{
			...
			new STAError(cur_client, x + Constants.STA_NICK_TAKEN,
					"Nick reserved. Please choose another.");
			return;
		}
		// now must check if hub is full...
		if (State.equals("PROTOCOL")) // otherwise is already connected, no
		// point in checking this
		{
			...
			{
				new STAError(cur_client, 200 + Constants.STA_HUB_FULL,
						"Hello there. Hub is full, there are "
								+ String.valueOf(i) + " users online.\n"
								+ Vars.Msg_Full);
				return;
			}
		...
		if (!cur_client.reg.overridespam)
			if (cur_client.EM != null)
				...
				{
					new STAError(cur_client, State.equals("PROTOCOL") ? 200
							: 100, "E-mail contains forbidden words.");
					return;
				}
		if (!cur_client.reg.overridespam)
			if (cur_client.DE != null)
				...
				{
					new STAError(cur_client, State.equals("PROTOCOL") ? 200
							: 100, "Description contains forbidden words");
					return;
				}
		...
		if (!cur_client.reg.overridespam)
			if (cur_client.SL == null && Vars.min_sl != 0)
				new STAError(cur_client,
						200 + Constants.STA_REQUIRED_INF_FIELD_BAD_MISSING,
						"Too few slots, open up more.", "FB", "SL");
				...
				if (cur_client.SS != null)
					if (Long.parseLong(cur_client.SS) < 1024 * Vars.min_share * 1024)
					{
						new STAError(
								cur_client,
								200 + Constants.STA_REQUIRED_INF_FIELD_BAD_MISSING,
								"Share too small " + Vars.min_share
										+ " MiB required.", "FB", "SS");
						return;
					}
			if (!cur_client.reg.overrideshare)
				...
				if (cur_client.SL != null)
					if (Integer.parseInt(cur_client.SL) > Vars.max_sl)
					{
						new STAError(
								cur_client,
								200 + Constants.STA_REQUIRED_INF_FIELD_BAD_MISSING,
								"Too many slots, close some.", "FB", "SL");
						return;
					}
			if (!cur_client.reg.overridespam)
				if (Integer.parseInt(cur_client.HN) > Vars.max_hubs_user)
				{
					new STAError(cur_client,
							200 + Constants.STA_REQUIRED_INF_FIELD_BAD_MISSING,
							"Too many hubs open, close some.", "FB", "HN");
					return;
				}
			if (!cur_client.reg.overridespam)
		...
		{
			new STAError(cur_client, 200 + Constants.STA_CID_TAKEN,
		...
		{
			new STAError(cur_client, 200 + Constants.STA_NICK_TAKEN,
					"Nick taken, please choose another");
			return;
		}
		...
		if (State.equals("PROTOCOL"))
			...
			{
				new STAError(cur_client, 200 + Constants.STA_INVALID_PID,
		...
		if (State.equals("PROTOCOL"))
		{
			if (cur_client.reg.isreg)
			{
				if (cur_client.reg.Password.equals(""))// no pass defined (
				// yet)
				{
					cur_client
							.sendToClient("ISTA 000 Registered,\\sno\\spassword\\srequired.\\sThough,\\sits\\srecomandable\\sto\\sset\\sone.");
				...
				myTiger.engineReset();
				myTiger.init();
				byte[] T = Long.toString(System.currentTimeMillis()).getBytes(); // taken
				// from
				// cur
				// time
			...
			} else
			{
				...
				{
					cur_client
							.sendToClient("ISTA 000 Nick\\sRegistered\\s(flyable\\saccount).\\sPlease\\sprovide\\spassword.");
				...
				{
					new STAError(cur_client, 200 + Constants.STA_REG_ONLY,
							"Registered only hub.");
					return;
				}
			}
		...
		if (State.equals("PROTOCOL"))
		{
			 ...
			 * (cur_client)) //if the user has some inf ... [ meaning he is ok]
			 * inf=inf.substring(0,inf.length()-1)+tempy.cur_client.getINF
			 * ()+"\n\n"; } inf=inf.substring(0,inf.length()-1)+"BINF DCBA
			 * ID"+Vars.SecurityCid+" NI"+ADC.retADCStr(Vars.bot_name) +" BO1
			 * OP1 DE"+ADC.retADCStr(Vars.bot_desc)+"\n";
			 * 
			 * inf+=cur_client.getINF (); //sending inf about itself too
			 * cur_client.sendToClient(inf);
			 */

			// ClientNod faulty=null;
			synchronized (SimpleHandler.Users)
			{
				// System.out.println("marimea este
				// "+SimpleHandler.Users.size());
			...
			// ok now must send INF to all clients
			Broadcast.getInstance().broadcast(cur_client.getINF(),
					cur_client.myNod);
			// System.out.println("acum am trimis ca a intrat "+cur_client.ID);

			// Main.PopMsg(cur_client.NI+" with SID "+cur_client.SessionID+"
			// just entered.");
			// cur_client.sendFromBot(""+Main.Server.myPath.replaceAll (" ","\\
			// "));
			// ok now that we passed to normal state and user is ok, check if it
			// has UCMD, and if so, send a test command
	...
	void HandleIssuedCommand() throws CommandException, STAException
	{
		...
		if (Issued_Command.substring(1).startsWith("INF"))
		{
			...
			{
				new STAError(cur_client, 200 + Constants.STA_INVALID_STATE,
						"INF Invalid State.", "FC", Issued_Command.substring(0,
								4));
				return;
			}
		...
		/** **********************PAS COMMAND*************************** */
		if (Issued_Command.charAt(1) == 'P' && Issued_Command.charAt(2) == 'A'
				&& Issued_Command.charAt(3) == 'S')
		{
			...
			if (Issued_Command.charAt(0) != 'H')
			{
				if (State.equals("NORMAL"))
					throw new CommandException(
							"FAIL state:PROTOCOL reason:NOT BASE CLIENT");
				else
				{
			...
			if (realpas.equals(Issued_Command.substring(5)))
			{
				...
				if (!cur_client.ID.equals(cur_client.reg.CID))
					cur_client
							.sendToClient("IMSG Account\\sCID\\supdated\\sto\\s"
									+ cur_client.ID);
				cur_client.reg.CID = cur_client.ID;
			} else
			{
				new STAError(cur_client, 200 + Constants.STA_INVALID_PASSWORD,
						"Invalid Password.");
				return;
			}
		...
		/** ********************SUP COMMAND***************************** */
		if (Issued_Command.charAt(1) == 'S' && Issued_Command.charAt(2) == 'U'
		 ...
		 * COMMAND***********************************
		 */
		if (Issued_Command.charAt(1) == 'M' && Issued_Command.charAt(2) == 'S'
				&& Issued_Command.charAt(3) == 'G')
		{
			new MSG(cur_client, State, Issued_Command);
		}

		if (Issued_Command.charAt(1) == 'S' && Issued_Command.charAt(2) == 'C'
				&& Issued_Command.charAt(3) == 'H')
		{
			new SCH(cur_client, Issued_Command, State);
		}
		if (Issued_Command.charAt(1) == 'S' && Issued_Command.charAt(2) == 'T'
				&& Issued_Command.charAt(3) == 'A')
		{
			new STA(cur_client, Issued_Command, State);
		}
		if (Issued_Command.substring(1).startsWith("RES ")) // direct search
		// result, only
		// active to passive
		// must send this
		{
			new RES(cur_client, State, Issued_Command);
		} else if (Issued_Command.substring(1).startsWith("CTM ")) // direct
		// connect
		// to me
		{
			new CTM(cur_client, State, Issued_Command);
		} else if (Issued_Command.substring(1).startsWith("RCM ")) // reverse
		// connect
		// to me
		{
	...
	/**
	 * Creates a new instance of Command with following params CH of type
	 * ClientHandler identifies tha client to handle Issued_command of String
	 * type actually identifies the given command state also of type String
	 * Identifies tha State in which tha connection is, meaning [ accordingly to
	 * arne's draft]: PROTOCOL (feature support discovery), IDENTIFY (user
	 * identification, static checks), VERIFY (password check), NORMAL (normal
	 * operation) and DATA (for binary transfers). Calling function should send
	 * one of this params, that is calling function request... Command class
	 * does not check params. Function throws CommandException if smth is wrong.
	 */
			...
			throws ClientFailedException, STAException, CommandException
	{
		...
		{
			// System.out.println("("+cur_client.NI+")"+System.currentTimeMillis
			// ()/1000);
			return;
		}
