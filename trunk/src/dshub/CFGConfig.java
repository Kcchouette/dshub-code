		...
		if (recvbuf.equals("cfg")) {
			...
			if (aux.toLowerCase().equals("timeout_login")) {
				...
				if (!HostTester.hostOK(new_name)) {
					cur_client
							.sendFromBot("The hub_host you provided does not point to one of your eth interfaces. "
				...
				try {
					...
					cur_client
							.sendFromBot("Invalid proxy. Possible reasons: incorrect input, domain resolution failure, invalid proxy.");
				}
